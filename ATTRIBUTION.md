...
`tools/frozen/transform_from_earlier.py` rebuilds `ozzit.xlsx` from Ryan Duguid's earlier workbook of 6 July 2024, which is not committed to this repository. The build was last run against a file of 1,478,643 bytes, sha256 `f38dbc83b4a18fc7d71d0f4bcf39680d74694b9aa129f5b3deb39b014e0bbb67`, holding 224 parts. A rebuild that starts from that file reproduces the v3.0.0 `src/` and `functions.csv`; anything else is a different input and the build's own assertions will say so. The transform does not take `ozzit.xlsx` as a substitute for that earlier file.

That script is frozen. It is a one-shot migration: it ran once against an input this repository does not ship, it hardcodes the date it was run, and it stops at v3.0.0, so re-running it is neither possible here nor useful. It is kept under `tools/frozen/` for provenance, as the record of what the v3.0.0 transform did, and it is excluded from the lint, type-check and test gates for that reason. Nothing imports it and no gate runs it. Every later change goes through `tools/postbuild/`, which starts from the committed workbook.
