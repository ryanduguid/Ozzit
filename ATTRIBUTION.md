...
- All earlier namespaces were replaced by a single `oz.` prefix, with a one-letter tag where two modules shared a function name. The earlier workbook branding, branded artwork, the cover video thumbnails and their YouTube link, and Dropbox file links were removed. One maths-citation link (a Diarmuid Early video, credited in an `IntOnIntλ` source comment) was retained deliberately.
