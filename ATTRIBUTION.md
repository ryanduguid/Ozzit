...
That claim describes the v3.0.0 baseline. v3.1.0's FY27 examples, Luma palette and later maintenance passes changed the built workbook after that baseline was produced, and the tracked repository tools now preserve and verify those changes rather than regenerate the historical pass from the earlier workbook. A rebuild from the earlier file reproduces the v3.0.0 artefacts, then requires those later passes to reach the current release. Byte-for-byte reproduction of the current workbook is not claimed: the FY27 date shift was Excel-state-dependent and an Excel save is not stable across Excel builds.
