...
//  Parameter Declaration
    ...
    //  A payment in advance is discounted one period less than the same payment in arrears
        Exponents,      SEQUENCE(1, COLUMNS(Flat)) - IF(Advance?, 1, 0),
        Result,         SUM(Flat / (1 + Rate) ^ Exponents),
...
//  Parameter Declaration
    ...
    //  Help
        Help,           TRIM(TEXTSPLIT(
                            ...
                            "DESCRIPTION:   →Unwinds an AASB 16 lease liability over its term.¶NOTES!         →Returns four rows: opening liability, payment, interest, closing¶               →liability. Interest accrues on the opening balance in arrears and on¶               →the balance after the payment in advance, so closing is opening less¶               →payment plus interest either way and the last closing balance is nil.¶               →The opening balance is LeaseLiabilityλ() on the same arguments. Pair¶               →this with ROUScheduleλ() for the asset that sits opposite it: the two¶               →together give the front-loaded expense profile AASB 16 produces.¶               →Arithmetic only, on the payments and rate you supply.¶" &
                            "WEBPAGE:       →https://github.com/ryanduguid/Ozzit¶" &
                            "VERSION:       →24 Aug 2026¶" &
                            "PARAMETERS:    →¶" &
                            "Payments       →(Required) One row or one column of lease payments, one per period, entered positive.¶" &
    ...
    //  One lease at a time, laid along a row the way the timeline helpers expect
        Flat,           IF(ROWS(Payments) = 1, Payments, TRANSPOSE(Payments)),
        Count,          COLUMNS(Flat),
        Shift,          IF(Advance?, 1, 0),
        Opening1,       LeaseLiabilityλ(Flat, Rate, Advance?),
    //  A payment in advance leaves before interest accrues; one in arrears leaves after
        Closings,       SCAN(Opening1, Flat, LAMBDA(Balance, Payment,
                            (Balance - Shift * Payment) * (1 + Rate) - (1 - Shift) * Payment)),
    //  Each opening is the closing before it. A one-period lease has no closing to shift.
        ...
        Interest,       (Openings - Shift * Flat) * Rate,
        Result,         VSTACK(Openings, Flat, Interest, Closings),
    //  Return Result or Help
        CHOOSE( Help? + 1, Result, Help)
    )
);'''
