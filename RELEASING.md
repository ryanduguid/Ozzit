...
The current acceptance baseline is 1,129 formulas recalculated with zero error cells and 912 assertions run with zero failures: the 610 hand-written assertions, 23 of them randomised checks that draw fresh inputs on every run and name the inputs behind any miss, and the 302 that `tools/generate_selftest_examples.py` derives from the help. The generated count is a static count of the fragment; record the number the script prints.
