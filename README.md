...
| `tools/` | The build and maintenance pipeline: `transform_from_earlier.py` rebuilds the v3.0.0 baseline from an uncommitted earlier workbook and stops there; later tracked postbuild passes start from the committed `ozzit.xlsx` and `src/` (see ATTRIBUTION.md), then the AFE store is synchronised, the workbook is canonicalised/polished, and the gates check it |
